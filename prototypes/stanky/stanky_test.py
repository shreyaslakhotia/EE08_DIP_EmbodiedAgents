...
class RemoteBrain:
    ...
    def generate_response_stream(self, user_text, image_path=None):
        ...
        # Update the system prompt with current time and instructions
        if self.history and self.history[0]['role'] == 'system':
            self.history[0]['content'] = system_injection
        ...
        for attempt in range(max_retries):
            full_reply = "" # Reset full_reply for each attempt to keep history clean
            try:
...
class StudyBuddyApp:
    ...
    def process_ai_stream(self, text):
        try:
            ...
            # Use case-insensitive regex to catch all variations (e.g., "Follow me", "*FOLLOW ME*")
            speech_text = re.sub(r'\*?FOLLOW ME\*?', '', full_reply, flags=re.IGNORECASE)
            speech_text = re.sub(r'\*?STOP\*?', '', speech_text, flags=re.IGNORECASE)
            ...
            if not self.processing and self.current_frame_img:
                ...
                try:
                    ...
                    if analysis and "SILENCE" not in analysis.upper():
                        ...
                        self.audio.speak(analysis, on_start=self.face.start_talking, on_end=self._tts_finish_face_state) 
